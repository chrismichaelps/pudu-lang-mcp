---
type: module
path: "@root/src/Services/Reference.pudu"
fidelity: Active
...
- **Requires:** [[src/Services/Toolchain]], [[src/Domain/Reference/Decode]], [[src/Domain/Code/SourceFile]], [[src/Constants/Server]], `Std.Concurrent`, `Std.Fs`, `Std.Io`, `Std.Path`.
- **Consumed by:** [[src/App/Tools/Reference]], [[src/App/Resources]], [[src/App/Completion]].
...
- `build`: split the files into chunks of `REFERENCE_CHUNK` and, with at most `REFERENCE_WORKERS` at
  ...
  `REFERENCE_TIMEOUT_MS` and `REFERENCE_OUTPUT_CAP_BYTES`). Decode each chunk ([[src/Domain/Reference/Decode]]) and join the chunks,
