---
type: module
path: "@root/src/App/Tools/Language.pudu"
fidelity: Active
domain: "[[domain/Tool]]"
...
- **Requires:** [[src/App/Context]], [[src/Services/Lsp]], [[src/Services/Workspace]], [[src/Domain/Lsp/Conversation]], [[src/Domain/Lsp/Render]], [[src/Domain/Catalog/Arguments]], [[src/Utils/Uri]], [[src/Utils/TextBounds]].
- **Consumed by:** [[src/App/Tools/Registry]].

## Algorithm

`ask` places the code ([[src/Services/Workspace]]), builds the question from `line`, `character`,
and `newName`, and asks [[src/Services/Lsp]] with the file's URI and the target directory as the
root. A language-server error reply answers "The language server answered: <message>". Each tool
renders the result with [[src/Domain/Lsp/Render]]; paths are shown by `shownPath`:
